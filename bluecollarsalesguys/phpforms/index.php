...
// Sessions support                                   //
//----------------------------------------------------//
if (strtoupper( $aCFG["SESSION_ALLOW"] )=='TRUE')
{
    /* {{{ Create visitor session }}} */
    @require_once( INC . 'session.class' . PHP_EXT );
    $oSESS = new SESSION( $aCFG["SESSION_STORAGE"] );
  
    $CNK->__sSessionId = $oSESS->SessionStart();
}
...
{
    $CNK->__sBaseUrl = 
      $CNK->__aCfg["URL_BASE"] . 'index' . PHP_EXT  . '?'.$aCFG["SESSION_IDENTID"].'='.$CNK->__sSessionId;
}
else 
{
    $CNK->__sBaseUrl = 
      $CNK->__aCfg["URL_BASE"] . 'index' . PHP_EXT . '?'.$aCFG["SESSION_IDENTID"].'=0';
}
